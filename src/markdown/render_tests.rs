...
use crate::markdown::{MarkdownBlock, MarkdownRenderer};
...
impl RichTextTheme for TestTheme {
    ...
    fn get_info_color(&self) -> Color { Color::Blue }
    ...
    fn get_focused_border_color(&self) -> Color { Color::Cyan }
    fn get_secondary_color(&self) -> Color { Color::Yellow }
    fn get_background_color(&self) -> Color { Color::Black }
    fn get_json_key_color(&self) -> Color { Color::Cyan }
    fn get_json_string_color(&self) -> Color { Color::Green }
    fn get_json_number_color(&self) -> Color { Color::Magenta }
    fn get_json_bool_color(&self) -> Color { Color::Yellow }
...
fn paragraph_renders_text() {
    ...
    assert_eq!(lines.len(), 1);
    let buf = render_to_buffer(lines, 80, 5);
    let text: String = (0..13).map(|x| buf.cell((x, 0)).unwrap().symbol()).collect();
...
fn blank_line_produces_empty_line() {
    ...
    let blank_idx = lines.iter().position(|l| l.spans.is_empty() || l.spans.iter().all(|s| s.content.is_empty()));
    assert!(blank_idx.is_some(), "expected a blank line between two paragraphs");
}
...
fn code_block_with_lang_renders_bordered_box() {
    ...
    assert!(lines.len() >= 3, "expected header, content, footer; got {} lines", lines.len());
    let buf = render_to_buffer(lines, 80, 5);
    assert_eq!(buf.cell((0, 0)).unwrap().symbol(), "╭");
}
...
fn mermaid_code_block_is_skipped() {
    let md = "```mermaid\ngraph TD\nA-->B\n```";
    let lines = render_markdown(md, 80);
    assert!(lines.is_empty(), "mermaid blocks should produce zero output lines");
}
...
fn nested_list_indents() {
    ...
    let inner_block = blocks.iter().find(|b| matches!(b, MarkdownBlock::ListItem(t, _) if t == "inner"));
    assert!(inner_block.is_some(), "should find inner list item");
    if let Some(MarkdownBlock::ListItem(_, indent)) = inner_block {
        assert_eq!(*indent, 1, "inner list item should have indent=1, got {}", indent);
    }
...
fn table_renders_with_borders() {
    let md = "| A | B |\n|---|---|\n| 1 | 2 |";
    let lines = render_markdown(md, 80);
    assert!(lines.len() >= 4, "expected top border, header, separator, row, bottom; got {}", lines.len());
    let buf = render_to_buffer(lines, 80, 10);
    assert_eq!(buf.cell((0, 0)).unwrap().symbol(), "┌");
}
...
fn inline_bold_italic_renders() {
    ...
    assert!(spans[0].style.add_modifier.contains(Modifier::BOLD | Modifier::ITALIC));
}
...
fn mixed_inline_formatting() {
    let spans = crate::markdown::parse_inline_formatting("normal **bold** *italic* `code`", &TestTheme);
    assert!(spans.len() >= 4, "expected at least 4 spans for mixed formatting");
}
...
"#;
    let lines = render_markdown(md, 80);
    assert!(lines.len() > 10, "complex document should produce many lines, got {}", lines.len());
...
fn code_block_preserves_content() {
    ...
    let lines = render_markdown(md, 80);
    let buf = render_to_buffer(lines, 80, 10);
    let row1: String = (0..20).map(|x| buf.cell((x, 1)).unwrap().symbol()).collect();
...
fn multiple_paragraphs_separated_by_blank() {
    ...
    let lines = render_markdown(md, 80);
    let blank_count = lines.iter().filter(|l| l.spans.is_empty() || l.spans.iter().all(|s| s.content.is_empty())).count();
